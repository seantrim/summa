...
! look-up values for the choice of time stepping order of accuracy (SJT)
integer(i4b),parameter,public :: firstOrder           = 321    ! Implicit Euler (SJT)
integer(i4b),parameter,public :: secondOrder          = 322    ! Implicit Midpoint (SJT)
...
contains
 ...
 ! choice of time stepping order of accuracy (SJT)
 select case(trim(model_decisions(iLookDECISIONS%tStepOrder)%cDecision))
  case('firstOrder');                  model_decisions(iLookDECISIONS%tStepOrder)%iDecision = firstOrder          ! Implicit Euler
  case('secondOrder');                 model_decisions(iLookDECISIONS%tStepOrder)%iDecision = secondOrder         ! Implicit Midpoint
  case default
   err=10; message=trim(message)//"unknown option for time stepping [option="//trim(model_decisions(iLookDECISIONS%tStepOrder)%cDecision)//"]"; return
 end select

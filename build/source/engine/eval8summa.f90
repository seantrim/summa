...
subroutine eval8summa_cvode(&
  ...
  real(rkind),intent(in)          :: scalarCanopyTemp        ! previous value for temperature of the vegetation canopy (K)
  ...
  real(rkind),intent(in)          :: scalarCanopyLiq         ! previous value for mass of ice on the vegetation canopy (kg m-2)
  real(rkind),intent(out)         :: scalarCanopyEnthalpyTrial   ! trial value for enthalpy of the vegetation canopy (J m-3)
  real(rkind),intent(in)          :: scalarCanopyEnthalpy    ! previous value of enthalpy of the vegetation canopy (J m-3)
  real(rkind),intent(out)         :: mLayerTempTrial(:)          ! trial vector of layer temperature (K)
  real(rkind),intent(in)          :: mLayerTemp(:)           ! previous vector of layer temperature (K)
  ...
  real(rkind),intent(in)          :: mLayerMatricHead(:)     ! previous value for total water matric potential (m)
  real(rkind),intent(out)         :: mLayerVolFracWatTrial(:)    ! trial vector of volumetric total water content (-)
  real(rkind),intent(in)          :: mLayerVolFracWat(:)     ! previous vector of volumetric total water content (-)
  real(rkind),intent(out)         :: mLayerVolFracIceTrial(:)    ! trial vector of volumetric ice water content (-)
  real(rkind),intent(in)          :: mLayerVolFracIce(:)     ! previous vector of volumetric ice water content (-)
  real(rkind),intent(out)         :: mLayerVolFracLiqTrial(:)    ! trial vector of volumetric liquid water content (-)
  real(rkind),intent(in)          :: mLayerVolFracLiq(:)     ! previous vector of volumetric liquid water content (-)
  real(rkind),intent(out)         :: scalarAquiferStorageTrial   ! trial value of storage of water in the aquifer (m)
  real(rkind),intent(in)          :: scalarAquiferStorage    ! previous value of storage of water in the aquifer (m)
  real(rkind),intent(in)          :: mLayerEnthalpy(:)       ! previous vector of enthalpy for snow+soil layers (J m-3)
  ...
  !  mLayerEnthalpy          => diag_data%var(iLookDIAG%mLayerEnthalpy)%dat            ,& ! intent(in):  [dp(:)] enthalpy of the snow+soil layers (J m-3)
   ...
   !endif
    ...
    ! compute the residual vector
       !if (.not.insideCVODE) then
       !prog_data%var(iLookPROG%scalarCanairTemp)%dat(1) = scalarCanairTempTrial
    print*, 'rAdd', resSink(1:5)
    call computePrime(dt_cur, fluxVec + resSink/dt_cur, &
       resVec, &
       sMul, &
       indx_data, &
       diag_data, nLayers, scalarCanopyCmTrial, mLayerCmTrial, mLayerMatricHeadTrial, mpar_data, mLayerVolFracWatTrial, scalarCanopyIce, scalarCanopyIceTrial, mLayerVolFracIce, mLayerVolFracIceTrial, mLayerDepth, flux_data)
 !endif
  ! end association with the information in the data structures
  end associate
end subroutine eval8summa_cvode
...
integer(c_int) function eval8summa4cvode(tres, sunvec_y, sunvec_yp, user_data) &
  ...
  rVec(:) = 0
  dt_cur(1) = 1._rkind
  print*, eqns_data%ida_mem, dt_cur
  ierr = FCVODEGetCurrentStep(eqns_data%ida_mem, dt_cur)
  print*, dt_cur
  if (dt_cur(1) == 0) then
     dt_cur(1) = 1._rkind
  endif
  dt_cur(1) = merge(1._rkind, tres, tres==0)
  print*, 'dt', dt_cur
  ! compute the flux vector for a given state vector
  call eval8summa_cvode(&
                ! input: model control
       dt_cur(1),                      & ! intent(in):    data step
       eqns_data%dt, &
                eqns_data%nSnow,                   & ! intent(in):    number of snow layers
                eqns_data%nSoil,                   & ! intent(in):    number of soil layers
                eqns_data%nLayers,                 & ! intent(in):    number of layers

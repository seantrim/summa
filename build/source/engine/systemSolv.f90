...
subroutine systemSolv(&
  ...
  USE summaSolve4arkode_module,only:summaSolve4arkode       ! solve DAE by ARKODE
#endif
  ...
  associate(ixNumericalMethod => model_decisions(iLookDECISIONS%num_method)%iDecision) ! intent(in): [i4b] choice of numerical solver
    select case(ixNumericalMethod)
      case(ida)    ! solve for general time step using IDA
       if (ARKODE_flag) then ! replace with model decision in future update -- test flag for now
        call solve_with_ARKODE; if (return_flag) return           ! solve using ARKODE -- return if error
       else
...
contains
 ...
 subroutine solve_with_ARKODE
  ...
  !---------------------------
  ! * solve My' = fE(t,y) + fI(t,y) using ARKODE (y is the state vector, y'=dy/dt) *
  !---------------------------
  associate(&
   nSnow => indx_data%var(iLookINDEX%nSnow)%dat(1),& ! intent(in): [i4b] number of snow layers
   nSoil => indx_data%var(iLookINDEX%nSoil)%dat(1) & ! intent(in): [i4b] number of soil layers
  ...
  ! check if ARKODE is successful, only fail outright in the case of a non-recoverable error
  if ( .not.sunSucceeds ) then
   ...
   if (tooMuchMelt) then
    return_flag=.true.; return ! exit to start same step over after merge
   end if
  end if
 ...
 subroutine nested_Newton_iterations
  ...
  if (nested_Newton % nested) then ! nested iterations
   ...
   nested_Newton % kmax = 19999_i4b; nested_Newton % lmax = 0_i4b!10_i4b ! for state type decomposition
...
!   nested_Newton % out_SS4HG_inner    = nested_Newton % out_SS4HG ! allocate
   ...
   allocate(nested_Newton % in_SS4HG % xkp1lp1, mold = nested_Newton % xkp1lp1)
   allocate(nested_Newton % io_SS4HG % stateVecTrialNested(1:2_i4b*nState))
   allocate(nested_Newton % io_SS4HG % stateVecNewNested(1:2_i4b*nState))
   ...
   allocate(nested_Newton % io_SS4HG % aJac1(nested_Newton % in_SS4HG % nLeadDim,nested_Newton % in_SS4HG % nState))
   allocate(nested_Newton % io_SS4HG % aJac2(nested_Newton % in_SS4HG % nLeadDim,nested_Newton % in_SS4HG % nState))
   allocate(nested_Newton % io_SS4HG % dMat, source = nested_Newton % dMat) ! note: allocate and initialize
